# Harnessed：为什么独立验证很重要

## 问题

AI 编程代理会自己评估自己的工作——而且会系统性地判断失误。

这不是猜测。研究表明，LLM 存在**自偏好偏差**：当被要求评判输出时，它们会偏好与自己风格相似的回答，而不一定偏好实际质量更高的回答 [1][2]。在编程场景下，这意味着一个写了代码又审查自己代码的代理，在结构上就偏向于得出"代码是正确的"这一结论。

后果是具体的：
- 代理在需求不完整时就声明任务"完成"
- Bug 通过自审查，因为审查者和作者有相同的盲点
- 安全问题未被发现，因为模型对自己的输出缺乏对抗性视角

Harnessed 的存在就是为了打破这个循环。
...
### Harnessed 是：
- 一个降低 AI 编程代理自评偏差的系统
- 一个具有独立评估、显式合约和基于证据门控的结构化 QA 循环
- 一个施加比例审查的风险感知框架
- 对自身局限性和不确定性诚实的系统
...
Harnessed 的价值不在于让 AI 编程完美。价值在于让 AI 编程变得**可问责**——有显式标准、独立审查、具体证据和诚实的不确定性报告。
